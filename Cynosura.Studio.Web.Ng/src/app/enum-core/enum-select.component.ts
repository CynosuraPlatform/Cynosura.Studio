...
export class EnumSelectComponent implements OnInit, ControlValueAccessor,
    ...
    autocompleteEnums: Observable<Enum[]>;
    ...
    writeValue(value) {
        ...
        if (this.mode === 'autocomplete') {
            if (this.value) {
                this.enumService.getEnum({ solutionId: this.solutionId, id: this.value })
                    .subscribe(enm => this.autocompleteControl.setValue(enm));
            } else {
    ...
    ngOnInit(): void {
        if (this.mode === 'select') {
            ...
            this.autocompleteEnums = this.autocompleteControl.valueChanges
                .pipe(
                    map(value => {
                        if (value === '') {
                    ...
                    mergeMap(val => {
                        if (val.length !== 0) {
                            return this.enumService.getEnums({
