...
export class ViewSelectComponent implements OnInit, ControlValueAccessor,
    ...
    autocompleteViews: Observable<View[]>;
    ...
    writeValue(value) {
        ...
        if (this.mode === 'autocomplete') {
            if (this.value) {
                this.viewService.getView({ solutionId: this.solutionId, id: this.value })
                    .subscribe(view => this.autocompleteControl.setValue(view));
            } else {
    ...
    ngOnInit(): void {
        if (this.mode === 'select') {
            ...
            this.autocompleteViews = this.autocompleteControl.valueChanges
                .pipe(
                    map(value => {
                        if (value === '') {
